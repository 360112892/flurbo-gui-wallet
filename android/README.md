...
# Base environnement
        ...
        cd flurbo/utils/build_scripts
        docker build -f android32.Dockerfile -t flurbo-android .
        cd ..

# Build GUI

        cd android/docker
        docker build -t flurbo-gui-android .
        docker create -it --name flurbo-gui-android flurbo-gui-android bash

# Get the apk

        docker cp flurbo-gui-android:/opt/android/flurbo-gui/build/release/bin/bin/QtApp-debug.apk .
...
- Using ADB (Android debugger bridge) : 
  ...
  The only place where we are allowed to play is `/data/local/tmp`. So : 
        ...
        adb  push /opt/android/flurbo-gui/build/release/bin/bin/QtApp-debug.apk /data/local/tmp
