...
[Setup]
AppName=Flurbo GUI Wallet
; For InnoSetup this is the property that uniquely identifies the application as such
; Thus it's important to keep this stable over releases
; With a different "AppName" InnoSetup would treat a mere update as a completely new application and thus mess up

AppVersion=0.11.1.0
DefaultDirName={pf}\Flurbo GUI Wallet
DefaultGroupName=Flurbo GUI Wallet
UninstallDisplayIcon={app}\flurbo-wallet-gui.exe
...
; As of version 0.11.1.0, the Flurbo .exe files do not carry version info anyway in their .exe headers.
...
; applications somehow. Luckily this is no issue here because ALL files are "private" to Flurbo.

Source: "ReadMe.htm"; DestDir: "{app}"; Flags: ignoreversion
Source: "FinishImage.bmp"; Flags: dontcopy

; Flurbo GUI wallet
Source: "bin\flurbo-wallet-gui.exe"; DestDir: "{app}"; Flags: ignoreversion

; Flurbo GUI wallet log file
...
Source: "flurbo-wallet-gui.log"; DestDir: "{app}"; Flags: onlyifdoesntexist; Permissions: users-modify

; Flurbo CLI wallet
Source: "bin\flurbo-wallet-cli.exe"; DestDir: "{app}"; Flags: ignoreversion

; Flurbo wallet RPC interface implementation
Source: "bin\flurbo-wallet-rpc.exe"; DestDir: "{app}"; Flags: ignoreversion

; Flurbo daemon
Source: "bin\flurbod.exe"; DestDir: "{app}"; Flags: ignoreversion

; Flurbo daemon wrapped in a batch file that stops before the text window closes, to see any error messages
Source: "flurbo-daemon.bat"; DestDir: "{app}"; Flags: ignoreversion;

; Flurbo blockchain utilities
Source: "bin\flurbo-blockchain-export.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\flurbo-blockchain-import.exe"; DestDir: "{app}"; Flags: ignoreversion

; was present in 0.10.3.1, not present anymore in 0.11.1.0
; Source: "bin\flurbo-utils-deserialize.exe"; DestDir: "{app}"; Flags: ignoreversion
...
; Filename: "{app}\flurbo-wallet-gui.exe"; Description: "Run GUI Wallet now"; Flags: postinstall nowait skipifsilent
...
begin
  ...
  blockChainDefaultDir := ExpandConstant('{commonappdata}\bitflurbo');
  s := 'The default folder to store the Flurbo blockchain is ' + blockChainDefaultDir;
...
begin
  Result := BlockChainDir('') + '\bitflurbo.log';
...
begin
  if CurStep = ssPostInstall then begin
    // Re-build "flurbo-daemon.bat" according to actual install and blockchain directory used
    SetArrayLength(s, 3);
    s[0] := 'REM Execute the Flurbo daemon and then stay with window open after it exits';
    s[1] := '"' + ExpandConstant('{app}\flurbod.exe') + '" ' + DaemonFlags('');
    s[2] := 'PAUSE';
    SaveStringsToFile(ExpandConstant('{app}\flurbo-daemon.bat'), s, false); 
  end;
end;
...
[Icons]
; Icons in the "Flurbo GUI Wallet" program group
; Windows will almost always display icons in alphabetical order, per level, so specify the text accordingly
Name: "{group}\GUI Wallet"; Filename: "{app}\flurbo-wallet-gui.exe"
...
Name: "{group}\Utilities\Flurbo Daemon"; Filename: "{app}\flurbod.exe"; Parameters: {code:DaemonFlags}
...
Name: "{group}\Utilities\Textual (CLI) Wallet"; Filename: "{app}\flurbo-wallet-cli.exe"; WorkingDir: "{userdocs}\Flurbo\wallets"
...
Name: "{group}\Utilities\x (Check Default Wallet Folder)"; Filename: "{win}\Explorer.exe"; Parameters: "{userdocs}\Flurbo\wallets"
Name: "{group}\Utilities\x (Check GUI Wallet Log)"; Filename: "Notepad"; Parameters: "{app}\flurbo-wallet-gui.log"
Name: "{group}\Utilities\x (Try Daemon, Exit Confirm)"; Filename: "{app}\flurbo-daemon.bat"
Name: "{group}\Utilities\x (Try GUI Wallet Low Graphics Mode)"; Filename: "{app}\start-low-graphics-mode.bat"
Name: "{group}\Utilities\x (Try Kill Daemon)"; Filename: "Taskkill.exe"; Parameters: "/IM flurbod.exe /T /F"

; Desktop icons, optional with the help of the "Task" section
Name: "{userdesktop}\GUI Wallet"; Filename: "{app}\flurbo-wallet-gui.exe"; Tasks: desktopicon
...
Root: HKCU; Subkey: "Software\flurbo-project"; Flags: uninsdeletekeyifempty
Root: HKCU; Subkey: "Software\flurbo-project\flurbo-core"; Flags: uninsdeletekey
Root: HKCU; Subkey: "Software\flurbo-project\flurbo-core"; ValueType: string; ValueName: "daemonFlags"; ValueData: {code:DaemonFlags};
