...
{
    // prepare command line arguments and pass to flurbod
    ...
    qDebug() << "starting flurbod " + m_flurbod;
    ...
    // Start flurbod
    bool started = m_daemon->startDetached(m_flurbod, arguments);
    ...
    while(true && !m_app_exit) {
        ...
        if(running(testnet)) {
            ...
            if(counter >= 5) {
                qDebug() << "Killing it! ";
#ifdef Q_OS_WIN
                QProcess::execute("taskkill /F /IM flurbod.exe");
#else
                QProcess::execute("pkill flurbod");
#endif
            }
...
{ 
    ...
    qDebug() << status;
    // `./flurbod status` returns BUSY when syncing.
...
bool DaemonManager::sendCommand(const QString &cmd,bool testnet, QString &message) const
{
    ...
    p.start(m_flurbod, external_cmd);
    ...
    : QObject(parent)
{
    ...
    // Platform depetent path to flurbod
#ifdef Q_OS_WIN
    m_flurbod = QApplication::applicationDirPath() + "/flurbod.exe";
#elif defined(Q_OS_UNIX)
    m_flurbod = QApplication::applicationDirPath() + "/flurbod";
#endif

    if (m_flurbod.length() == 0) {
