...
class WalletListenerImpl : public  Flurbo::WalletListener
{
public:
...
{
    QFuture<Flurbo::Wallet::ConnectionStatus> future = QtConcurrent::run(m_walletImpl, &Flurbo::Wallet::connected);
    QFutureWatcher<Flurbo::Wallet::ConnectionStatus> *connectionWatcher = new QFutureWatcher<Flurbo::Wallet::ConnectionStatus>();

    connect(connectionWatcher, &QFutureWatcher<Flurbo::Wallet::ConnectionStatus>::finished, [=]() {
        QFuture<Flurbo::Wallet::ConnectionStatus> future = connectionWatcher->future();
                                              ...
                                              PendingTransaction::Priority priority)
{
    ...
    Flurbo::PendingTransaction * ptImpl = m_walletImpl->createTransaction(
                dst_addr.toStdString(), payment_id.toStdString(), amount, mixin_count,
                static_cast<Flurbo::PendingTransaction::Priority>(priority), currentSubaddressAccount(), subaddr_indices);
                                                 ...
                                                 quint32 mixin_count, PendingTransaction::Priority priority)
{
    std::set<uint32_t> subaddr_indices;
    Flurbo::PendingTransaction * ptImpl = m_walletImpl->createTransaction(
                dst_addr.toStdString(), payment_id.toStdString(), Flurbo::optional<uint64_t>(), mixin_count,
                static_cast<Flurbo::PendingTransaction::Priority>(priority), currentSubaddressAccount(), subaddr_indices);
...
{
    Flurbo::PendingTransaction * ptImpl = m_walletImpl->createSweepUnmixableTransaction();
    PendingTransaction * result = new PendingTransaction(ptImpl, this);
    return result;
}
...
UnsignedTransaction * Wallet::loadTxFile(const QString &fileName)
{
    ...
    Flurbo::UnsignedTransaction * ptImpl = m_walletImpl->loadUnsignedTx(fileName.toStdString());
...
{
    return QString::fromStdString(Flurbo::Wallet::genPaymentId());
}
...
{
    return QString::fromStdString(m_walletImpl->getDefaultDataDir()) + "/bitflurbo.log";
}

QString Wallet::getWalletLogPath() const
{
    return QCoreApplication::applicationDirPath() + "/flurbo-wallet-gui.log";
}

Wallet::Wallet(Flurbo::Wallet *w, QObject *parent)
    : QObject(parent)
    , m_walletImpl(w)
    , m_history(nullptr)
...
Wallet::~Wallet()
{
    ...
    delete m_subaddress;
    m_subaddress = NULL;
    //Flurbo::WalletManagerFactory::getWalletManager()->closeWallet(m_walletImpl);
