...
if [ "$platform" == "darwin" ]; then
    BIN_PATH=$BIN_PATH/flurbo-wallet-gui.app/Contents/MacOS/
elif [ "$platform" == "mingw64" ] || [ "$platform" == "mingw32" ]; then
    FLURBOD_EXEC=flurbod.exe
fi
...
get_tag
popd
echo "var GUI_FLURBO_VERSION = \"$TAGNAME\"" >> version.js
...
$QMAKE ../flurbo-wallet-gui.pro "$CONFIG" || exit
$MAKE || exit 

# Copy flurbod to bin folder
if [ "$platform" != "mingw32" ] && [ "$ANDROID" != true ]; then
cp ../$FLURBO_DIR/bin/$FLURBOD_EXEC $BIN_PATH
fi
